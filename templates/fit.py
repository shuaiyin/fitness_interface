...
class BaseHandler(tornado.web.RequestHandler):
    ...
    def check_have_attend_by_uid(self,uid,eid):
        ...
        uid = str(uid)
        eid = str(eid)
        count_info = self.db.get("SELECT COUNT(*) AS num FROM fs_user_event WHERE uid=%s and eid=%s and status=0",uid,eid)
   ...
   #input a list and add the rank string into the list elment  
    def add_rank_string(self,input_list):
        for index,ele in enumerate(input_list):
            input_list[index]['rank_string'] = 'the rank of ' + str(index + 1)
   ...
   #get the group_user_list \ input the group_id  and it will return the list of uid in this group 
    ...
    def get_userinfo_via_search_param(self,search_param,uid):
        ...
        if not self.cacheRedis.exists('users:uid:' + uid):# if this user info is not exist then read from mysql and write to cache
            path = options.ipnet
            userinfo = self.db.get("SELECT username,password,tel,idcard,sex,nickname,CONCAT(%s,avatar) AS avatar,point FROM fs_users WHERE uid = %s",path,uid)
...
class TestHandler(BaseHandler):
    ...
    def get(self):
        self.judge_class_have_event(1,1)
    ...
    def get(self):
        ...
        if action == 'get_first_level':
            ...
            if not self.cacheRedis.exists(redis_key):
                try:
                    level0_event_sql = 'SELECT eid AS id,ename AS name,epayfee,group_max,group_min FROM fs_events WHERE gid=%s and status=0 and upid=0 '  % (gid) + find_sql
                    level0_event_list = self.db.query(level0_event_sql)
                    for index,event in enumerate(level0_event_list):
                        event['is_event'] = 1
                        level0_event_list[index] = event
                    level0_class_list =  self.db.query('SELECT id,name FROM fs_events_class WHERE level=0 and gid = %s',gid)
                    for index,classInfo in enumerate(level0_class_list):
                        classInfo['is_event'] = 0
                        level0_class_list[index] = classInfo
                    first_level_return = level0_class_list + level0_event_list
                    self.cacheRedis.set(redis_key,first_level_return)
                except:
                    raise tornado.web.HTTPError(500, "系统开小差了，请重新刷新")
            try:
                first_level_return = self.cacheRedis.get(redis_key)
            except:
                raise tornado.web.HTTPError(500, "系统开小差了，请重新刷新")
            self.return_param(1,0,eval(first_level_return),'成功')
            ...
            if not self.cacheRedis.exists(redis_key):
                try:
                    level1_event_sql = 'SELECT eid AS id,ename AS name,epayfee,group_max,group_min FROM fs_events WHERE  status=0 and upid= %s' % (id) + find_sql
                    ...
                    level1_class_list =  self.db.query('SELECT id,name FROM fs_events_class WHERE upid = %s',id)
                    for index,classInfo in enumerate(level1_class_list):
                        classInfo['is_event'] = 0
                        level1_class_list[index] = classInfo
                    second_level_return = level1_event_list + level1_class_list
                    self.cacheRedis.set(redis_key,second_level_return)
                except:
                    raise tornado.web.HTTPError(500, "系统开小差了，请重新刷新")
            try:
                second_level_return = self.cacheRedis.get(redis_key)
            except:
                raise tornado.web.HTTPError(500, "系统开小差了，请重新刷新")
            self.return_param(1,0,eval(second_level_return),'成功')
            ...
            if not self.cacheRedis.exists(redis_key):
                try:
                    ...
                    raise tornado.web.HTTPError(500, "系统开小差了，请重新刷新")
            try:
                level3_event_list = self.cacheRedis.get(redis_key)
            except:
                raise tornado.web.HTTPError(500, "系统开小差了，请重新刷新")
            self.return_param(1,0,eval(level3_event_list),'成功')
            ...
            if not float(epayfee):
                public_info['checkstatus'] = 1
            ...
            #jusity if the user has attend the game  
    ...
    def get(self):
        ...
        if action == 'front_page':
            ...
            if gtype == '2': #jianbuzou online personal sport 
                ...
                if int(game_info['gstarttime']) < int(time.time()):#when the game is start,but there not consider the end of the game 
                ...
                rank_url = options.ipnet + '/py/rank?action=get_jbz_person_group_rank&gid=' + gid + '&uid=' + uid + '&rank_param=day&attend_type=person'
                brief_intro_url = options.ipnet + '/py/gamemore?action=get_game_brief&gid=' + gid
                self.render('front'+gid+'.html',rank_url=rank_url,brief_intro_url=brief_intro_url)
...
class RankHandler(BaseHandler):
    ...
    def judge_attend_type(self,uid):
        """
        ...
        """
        try:  attend_info = self.db.get("SELECT group_id FROM fs_user_event WHERE uid=%s and status=0 and checkstatus=2",uid)
        except: return False
    ...
    def get_my_run(self,uid,eid,type):
        ...
        """
        sql = "SELECT * FROM %s WHERE uid=%s and eid=%s" % (self.type_table[type],uid,eid)
        ...
        if action == 'get_person_jbz_run_rank':#runing man online jianbuzou sport
            ...
            if have_attend:
                ...
                self.render('person_run_rank' + gid + '.html',have_attend=have_attend,person_popular_list=person_popular_list,\
                           my_run=my_run,my_rank=my_rank,rank_param=rank_param,rank_param_e2c = rank_param_e2c[rank_param])
            else:
                self.render('person_run_rank' + gid + '.html',have_attend=have_attend,person_popular_list=person_popular_list, \
                             rank_param=rank_param,rank_param_e2c = rank_param_e2c[rank_param])
            ...
            """
            attend_type_get = self.get_argument('attend_type','person') #param from url
            attend_type = self.judge_attend_type(uid)

            if attend_type is False: raise tornado.web.HTTPError(500, "系统异常")
            if attend_type_get == 'person':
                ...
                self.render('jbz_person_group_rank'+ gid + '.html',have_attend=have_attend,popular_list=person_popular_list,\
                                my_run=my_run,my_rank=my_rank,rank_param=rank_param,attend_type_get=attend_type_get)
                ...
                self.render('jbz_person_group_rank'+ gid + '.html',have_attend=have_attend,popular_list=group_popular_list,\
                                my_run=my_group_run,my_rank=my_group_rank,rank_param=rank_param,attend_type_get=attend_type_get)
...
class OrgHandler(BaseHandler):  #yinshuai
    def get(self):
        ...
        if action == 'get_org': # search the organizition
            ...
            gid = self.get_argument('gid')
            try:
                sql = "SELECT name FROM fs_org WHERE name LIKE '%%" + word + "%%'"  + ' AND gid = ' + gid
