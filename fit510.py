...
class BaseHandler(tornado.web.RequestHandler):
    ...
    def write_user_point_from(self,uid,pid):
        """ write user_point_from """
        try:
            write_data = {'pid':pid,'uid':uid,'time':int(time.time())}
    ...
    def judge_today_point_give(self,uid,pid):
        """ the attribute of some point may only give once a day ,so we should judge if the user has been given"""
        early_night_timestamp = self.get_early_night_timestamp()
        return self.find_db_sum('fs_point_from',uid=uid,pid=pid,time={'rule':'>','value':early_night_timestamp})
    ...
    def get(self):
        ...
        if action == 'front_page':
            ...
            if gtype == '2': #jianbuzou online personal sport 
                ...
                if int(uid):
                    if not self.judge_today_point_give(uid,1): #if the user has not given this type of point h
                        point_info = self.get_point_info(1) # this game user point id is 1
                        self.write_user_point_from(uid,1) #write the from of the point 
