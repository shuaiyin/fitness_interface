...
class BaseHandler(tornado.web.RequestHandler):
    ...
    def write_user_point_from(self,uid,pid,gid,point_num=False):
        ...
        try:
            ...
            title = '积分提示'
            self.send_sysinfo(uid,point_name,title)
    ...
    def find_db_sum(self,table,**query_dict):
        ...
        except: self.treat_except()
        return True if int(sum) else False
   ...
   #get the group_user_list \ input the group_id  and it will return the list of uid in this group 
    ...
    def get_userinfo_via_search_param(self,search_param,uid):
        ...
        if not self.cacheRedis.exists('users:uid:' + uid):# if this user info is not exist then read from mysql and write to cache
            path = options.ipnet
            userinfo = self.db.get("SELECT username,password,tel,idcard,login_times,sex,nickname,last_login,CONCAT(%s,avatar) AS avatar,point FROM fs_users WHERE uid = %s",path,uid)
    ...
    def get_access_token(self,uid):
        uid = str(uid)
        if self.cacheRedis.exists('users:uid:' + uid ): access_token = self.cacheRedis.hget('users:uid:' + uid,'access_token')
        else: access_token = self.find_one('fs_users',['access_token'],uid=uid)
        return access_token

    #send system info 
    def send_sysinfo(self,uid,content,title):
        try: self.db.execute("INSERT INTO fs_sysinfo (uid,content,title,time) values (%s,%s,%s,%s)",uid,content,title,str(int(time.time())))
        except Exception,e: self.treat_except(e)
...
class TestHandler(BaseHandler):
    def get(self):
        self.write_user_point_from(2,1,5,500)
        # uid = 25
...
class ScoreHandler(BaseHandler):
    def get_score_list(self,uid):
        try: score_list = self.find_some('fs_user_event',['escore','eusername','gid','ueid','eid','group_id'],uid=uid,status=0,escore={'rule':'<>','value':''})
        except Exception,e: self.treat_except(e)
        return score_list
...
class UserHandler(BaseHandler):
    ...
    def post(self):
        ...
        if action == 'change_password':
            ori_password = self.get_argument('ori_password')
...
class PointHandler(BaseHandler):
    ...
    def get_user_point_list(self,uid):
        try:  user_point_list = self.db.query("SELECT fp.time,fp.point_num,p.name FROM fs_point_from  AS fp LEFT JOIN fs_point AS p ON fp.pid=p.id WHERE fp.status=0 and fp.uid=%s",uid)
        except Exception,e: self.treat_except(e)
...
class LoginHandler(BaseHandler):
      def get(self):
          ...
          if version == '2':
              tel_get = self.get_argument('tel')
              password_get = self.get_argument('password')  
              if not self.find_db_sum('fs_users',tel=tel_get): 
                  self.return_param(0,0,{},'not register')
                  return
              user_info = self.find_one('fs_users',['password','uid','login_times'],tel=tel_get)
              if not user_info['password'] == password_get: 
                  self.return_param(0,1,{},'wrong password')
                  return 
              access_token = self.create_access_token(user_info['uid'])
              change_param = {'access_token':access_token,'login_times':user_info['login_times']+1,'last_login':int(time.time())}
              self.update_db('fs_users',change_param,{'tel':tel_get})
              access_token = self.get_access_token(user_info['uid'])
              self.return_param(1,0,access_token,'login success')
