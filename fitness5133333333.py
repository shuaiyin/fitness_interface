...
class BaseHandler(tornado.web.RequestHandler):
    ...
    def write_user_point_from(self,uid,pid,gid,point_num=False):
        """ write user_point_from,the default point_num is ,which means that the point of this kind of point is fixed """
        try:
...
class ScoreHandler(BaseHandler):
    def get_score_list(self,uid):
        try: score_list = self.find_some('fs_user_event',['escore','bonus','eusername','gid','ueid','eid','group_id'],uid=uid,status=0,escore={'rule':'<>','value':''})
        except Exception,e: self.treat_except(e)
        return score_list
